...
measles_map <- function(df, state) {
  ...
  alt_text <- glue::glue(
    "Map showing measles cases in {state} ({sel_val}) and surrounding states between January 1, 2026 and {format(Sys.Date(), '%B %d, %Y')}."
  )
...
dtap_vaccination_comparison_chart <- function(df_dtap, df, state_name, comp_year = 2024) {
  
  # Pick two largest-pop neighboring states, excluding the selected state
  neighboring_data <- get_neighboring_states(df, state_name) |>
    st_drop_geometry() |>
    filter(name != state_name) |>
    left_join(
    ...
    filter(!is.na(total_population)) |>
    slice_max(total_population, n = 2, with_ties = FALSE) |>
    pull(name)
  
  # Force the intended comparison set:
  # selected state + neighbors + United States
  target_geos <- unique(c(state_name, neighboring_data, "United States"))
  
  chart_data <- tibble::tibble(geography = target_geos) |>
    left_join(
      df_dtap |>
        filter(year == comp_year) |>
        mutate(
        ...
        select(geography, estimate_percent) |>
        distinct(geography, .keep_all = TRUE),
      by = "geography"
    ) |>
    mutate(
      geography_chr = geography,
      geography = factor(
        geography,
        levels = rev(target_geos)
      ),
      bar_fill = ifelse(geography_chr == state_name, "#002D72", "white"),
      txt_col = ifelse(geography_chr == state_name, "white", "black")
    )
  
  missing_geos <- chart_data |>
    filter(is.na(estimate_percent)) |>
    pull(geography_chr)
  
  if (length(missing_geos) > 0) {
    warning(
      "Missing DTaP comparison value for: ",
  ...
  ggplot(chart_data, aes(x = estimate_percent, y = geography)) +
    ...
    labs(
      title = glue::glue("Vaccination comparison ({comp_year})"),
      x = NULL,
      y = NULL,
      alt = alt_text
    ) +
