...
- `state_policies_raw.xlsx` is the curated state-policy workbook used for the briefs. Rows marked `1` in the inclusion column are displayed; enacted status is derived from its binary enacted/not-enacted field.
...
- `state_policies_final.csv` contains the policy rows manually selected for inclusion in the PDFs, with status simplified to enacted or not enacted.
