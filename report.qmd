...
priority_years <- c("2024-25","2023-24","2022-23","2021-22","2020-21","2019-20")
...
<<<<<<< HEAD
  ...
  mutate(estimate_percent = readr::parse_number(as.character(estimate_percent))) |>
  group_by(geography, school_year) |>
  summarise(
    estimate_percent = first(estimate_percent[!is.na(estimate_percent)], default = NA_real_),
    .groups = "drop"
  ) |>
  pivot_wider(
  ...
  mutate(
    estimate_percent = coalesce(`2024-25`, `2023-24`, `2022-23`, `2021-22`, `2020-21`, `2019-20`),
...
=======
    select(geography, school_year, estimate_percent) |>
    filter(
        (geography == "Montana" & school_year == "2020-21") |
            (geography != "Montana" & school_year == "2024-25")
    )
>>>>>>> 166a05b8e72cfd00e8adaade920eb1b5df15d1e5
...
if (
    !(state_title %in%
        c("Montana", "West Virginia", "Maine", "California", "New York"))
) {
    ...
    text_content <- glue(
        "{state_title}'s rate of non-medical exemptions among kindergartners during the 2024-25 school year {variation_text} the state's 2023-24 rate."
    )
} else {
    heading_ad <- ""
    additional_details <- ""
...
#eval(policies)
#box(inset: 10pt)[#source("Source: AIM, LegiScan")]
```
